...
import type { SourcePart } from '../../../skills/hogwash/scripts/sync/source.js'
import {
  contentRevision,
  SYNC_SOURCE_NAMES,
  sourceOf,
} from '../../../skills/hogwash/scripts/sync/source.js'
...
describe('sourceOf', () => {
  ...
  it('names a pack file and a snapshot file that are actually on disk', () => {
    // Source paths are relative to the skill root; sync runs with cwd there.
    const skillRoot = 'skills/hogwash'
    for (const name of SYNC_SOURCE_NAMES) {
      expect(existsSync(join(skillRoot, sourceOf(name).packPath))).toBe(true)
      expect(existsSync(join(skillRoot, sourceOf(name).snapshotPath))).toBe(true)
    }
  })
